error id: file://<WORKSPACE>/src/main/scala/category/VectorAsFunctor.scala:`<none>`.
file://<WORKSPACE>/src/main/scala/category/VectorAsFunctor.scala
...
non-local guesses:
	 ...
	 -scala/Predef.zero().
offset: 235
uri: file://<WORKSPACE>/src/main/scala/category/VectorAsFunctor.scala
text:
```scala
package catigory
...
object VectorAsFunctor {
  ...
  case class Lin[V: Field](as: List[V]) {
    ...
    def apply(xs: List[V]): V = {
      if (xs.size == as.size) (xs zip as).foldLeft(ze@@ro) { (s, ax) =>
        val (a, x) = ax
        s + a * x
      }
